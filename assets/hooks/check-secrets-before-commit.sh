...
if echo "$diff" | grep -Eiq "$pattern"; then
  ...
  echo "Falso positivo comum: placeholders em arquivos de exemplo (ex: 'Password=[sua-senha]') e ate LINHAS REMOVIDAS que contenham o padrao ja disparam isto, mesmo sem segredo real no lado novo do diff." >&2
  echo "'git commit --no-verify' NAO contorna este bloqueio — este hook intercepta o comando no nivel do Claude Code (PreToolUse:Bash), nao e um hook nativo do git. Se confirmar manualmente que nao ha segredo real, rode o commit direto no seu terminal (fora do Claude Code)." >&2
  exit 2
fi
